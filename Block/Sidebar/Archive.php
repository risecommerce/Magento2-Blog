 ...
 */
class Archive extends \Risecommerce\Blog\Block\Post\PostList\AbstractList
{
    use Widget;
    ...
    public function getTimeUrl($time)
    {
        return $this->_url->getUrl(
            date('Y-m', $time),
            \Risecommerce\Blog\Model\Url::CONTROLLER_ARCHIVE
        );
    }
    ...
    public function getTranslatedDate($time)
    {
        $format = $this->_scopeConfig->getValue(
            'rcblog/sidebar/archive/format_date',
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );

        return \Risecommerce\Blog\Helper\Data::getTranslatedDate($format, $time);
    }
}
