...
class TagClaud extends \Magento\Framework\View\Element\Template
{
     ...
     */
    const ANIMATED_ENABLED = 'rcblog/sidebar/tag_claud/animated';
    const TEXT_HIGHLIGHT_COLOR = 'rcblog/sidebar/tag_claud/text_highlight_color';
    const CLOUD_HEIGHT = 'rcblog/sidebar/tag_claud/cloud_height';
    const TAG_COUNT = 'rcblog/sidebar/tag_claud/tag_count';
     ...
     * @param \Risecommerce\Blog\Model\ResourceModel\Tag\CollectionFactory $_tagCollectionFactory
     * @param array $data
     */
        ...
        if ($this->_tags === null) {
            ...
            $resource = $this->_tags->getResource();
                $this->_tags->getSelect()->joinLeft(
                    ['pt' => $resource->getTable('risecommerce_blog_post_tag')],
                    'main_table.tag_id = pt.tag_id',
                    []
                )->joinLeft(
                    ['p' => $resource->getTable('risecommerce_blog_post')],
                    'p.post_id = pt.post_id',
                    []
                )->joinLeft(
                    ['ps' => $resource->getTable('risecommerce_blog_post_store')],
                    ...
                    \Risecommerce\Blog\Model\Tag::STATUS_ENABLED
                )->order('count DESC');
    ...
    protected function _prepareLayout()
    {
        ...
        if ($this->getConfigValue(self::ANIMATED_ENABLED)) {
            $this->setTemplate('Risecommerce_Blog::sidebar/tag_claud_animated.phtml');
        }
