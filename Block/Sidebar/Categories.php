...
class Categories extends Template implements IdentityInterface
{
     ...
     * @param \Risecommerce\Blog\Model\ResourceModel\Category\Collection $categoryCollection
     * @param array $data
     */
    public function __construct(
        \Magento\Framework\View\Element\Template\Context $context,
        \Risecommerce\Blog\Model\ResourceModel\Category\Collection $categoryCollection,
        array $data = []
    ) {
    ...
    public function showPostsCount()
    {
        ...
        if (!$this->hasData($key)) {
            $this->setData($key, (bool)$this->_scopeConfig->getValue(
                'rcblog/sidebar/'.$this->_widgetKey.'/show_posts_count',
    ...
    public function maxDepth()
    {
        $maxDepth = $this->_scopeConfig->getValue(
            'rcblog/sidebar/'.$this->_widgetKey.'/max_depth',
            ScopeInterface::SCOPE_STORE
        );
