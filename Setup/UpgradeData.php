...
use Risecommerce\Blog\Model\ResourceModel\Comment;
    ...
    public function upgrade(ModuleDataSetupInterface $setup, ModuleContextInterface $context)
    {
        ...
        if (version_compare($version, '2.9.1') < 0) {
            ...
            $postSelect = $connection->select()->from(
                [$this->commentResource->getTable('risecommerce_blog_post')]
            )
                ->where('is_active = ?', 1);
        ...
        if (version_compare($version, '2.9.8') < 0) {
            ...
            $connection->delete(
                $this->commentResource->getTable('risecommerce_blog_tag_store'),
                ['store_id = ?' => 0]
            );

            $tagSelect = $connection->select()->from(
                [$this->commentResource->getTable('risecommerce_blog_tag')]
            );
