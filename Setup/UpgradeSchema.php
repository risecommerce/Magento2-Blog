...
class UpgradeSchema implements UpgradeSchemaInterface
{
    ...
    public function upgrade(SchemaSetupInterface $setup, ModuleContextInterface $context)
    {
        ...
        if (version_compare($version, '2.0.1') < 0) {
            foreach (['risecommerce_blog_post_relatedpost', 'risecommerce_blog_post_relatedproduct'] as $tableName) {
                // Get module table
            ...
            $connection->addColumn(
                $setup->getTable('risecommerce_blog_post'),
                'featured_img',
                [
        ...
        if (version_compare($version, '2.2.0') < 0) {
            ...
            $connection->addIndex(
                $setup->getTable('risecommerce_blog_post'),
                $setup->getIdxName($setup->getTable('risecommerce_blog_post'), ['author_id']),
                ['author_id']
            );
        }
        ...
        if (version_compare($version, '2.2.5') < 0) {
            ...
            foreach (['risecommerce_blog_post', 'risecommerce_blog_category'] as $table) {
        ...
        if (version_compare($version, '2.3.0') < 0) {
            ...
            $connection->addColumn(
                $setup->getTable('risecommerce_blog_category'),
                'meta_title',
                [
                    'type' => \Magento\Framework\DB\Ddl\Table::TYPE_TEXT,
                    'length' => 255,
            ...
            /**
             * Create table 'risecommerce_blog_tag'
             */
            $table = $setup->getConnection()->newTable(
                $setup->getTable('risecommerce_blog_tag')
            )->addColumn(
                'tag_id',
                ...
                $setup->getIdxName('risecommerce_blog_tag', ['identifier']),
                ['identifier']
            )->setComment(
                'Risecommerce Blog Tag Table'
            );
            $setup->getConnection()->createTable($table);

            /**
             * Create table 'risecommerce_blog_post_tag'
             */
            $table = $setup->getConnection()->newTable(
                $setup->getTable('risecommerce_blog_post_tag')
            )->addColumn(
                'post_id',
                \Magento\Framework\DB\Ddl\Table::TYPE_INTEGER,
                null,
                ...
                $setup->getIdxName('risecommerce_blog_post_tag', ['tag_id']),
                ['tag_id']
            )->addForeignKey(
                $setup->getFkName('risecommerce_blog_post_tag', 'post_id', 'risecommerce_blog_post', 'post_id'),
                'post_id',
                $setup->getTable('risecommerce_blog_post'),
                'post_id',
                \Magento\Framework\DB\Ddl\Table::ACTION_CASCADE
            )->addForeignKey(
                $setup->getFkName('risecommerce_blog_post_tag', 'tag_id', 'risecommerce_blog_tag', 'tag_id'),
                'tag_id',
                $setup->getTable('risecommerce_blog_tag'),
                'tag_id',
                \Magento\Framework\DB\Ddl\Table::ACTION_CASCADE
            )->setComment(
                'Risecommerce Blog Post To Category Linkage Table'
            );
            $setup->getConnection()->createTable($table);
        }
        ...
        if (version_compare($version, '2.5.2') < 0) {
            ...
            $connection->addIndex(
                $setup->getTable('risecommerce_blog_post'),
                $setup->getIdxName($setup->getTable('risecommerce_blog_post'), ['views_count']),
                ['views_count']
            );

            $connection->addIndex(
                $setup->getTable('risecommerce_blog_post'),
                $setup->getIdxName($setup->getTable('risecommerce_blog_post'), ['is_recent_posts_skip']),
                ['is_recent_posts_skip']
            );
        }
        ...
        if (version_compare($version, '2.6.0') < 0) {
            /**
             * Create table 'risecommerce_blog_comment'
             */
            $table = $setup->getConnection()->newTable(
                $setup->getTable('risecommerce_blog_comment')
            )->addColumn(
                'comment_id',
                \Magento\Framework\DB\Ddl\Table::TYPE_INTEGER,
                null,
                ...
                $installer->getIdxName('risecommerce_blog_comment', ['parent_id']),
                ['parent_id']
            )->addIndex(
                $installer->getIdxName('risecommerce_blog_comment', ['post_id']),
                ['post_id']
            )->addIndex(
                $installer->getIdxName('risecommerce_blog_comment', ['customer_id']),
                ['customer_id']
            )->addIndex(
                $installer->getIdxName('risecommerce_blog_comment', ['admin_id']),
                ['admin_id']
            )->addIndex(
                $installer->getIdxName('risecommerce_blog_comment', ['status']),
                ['status']
            )->addForeignKey(
                $installer->getFkName('risecommerce_blog_comment', 'post_id', 'risecommerce_blog_post', 'post_id'),
                'post_id',
                $installer->getTable('risecommerce_blog_post'),
                'post_id',
                \Magento\Framework\DB\Ddl\Table::ACTION_CASCADE
            );
            $setup->getConnection()->createTable($table);
        ...
        if (version_compare($version, '2.6.2') < 0) {
            ...
            $connection->addIndex(
                $setup->getTable('risecommerce_blog_category'),
                $setup->getIdxName($setup->getTable('risecommerce_blog_category'), ['include_in_menu']),
                ['include_in_menu']
            );
        }
        ...
        if (version_compare($version, '2.8.0') < 0) {
            ...
            $table = $setup->getTable('risecommerce_blog_tag');
            $connection->addColumn(
                $setup->getTable($table),
                'page_layout',
                [
        ...
        if (version_compare($version, '2.8.3.1') < 0) {
            // Fix issue https://github.com/risecommerce/module-blog/issues/205
            $table = $setup->getTable('risecommerce_blog_post');
        ...
        */
        ...
        if (version_compare($version, '2.9.8') < 0) {
            /**
             * Create table 'risecommerce_blog_tag_store'
             */
            $table = $connection->newTable(
                $setup->getTable('risecommerce_blog_tag_store')
            )->addColumn(
                'tag_id',
                \Magento\Framework\DB\Ddl\Table::TYPE_INTEGER,
                null,
                ...
                $setup->getIdxName('risecommerce_blog_tag_store', ['store_id']),
                ['store_id']
            )->addForeignKey(
                $setup->getFkName('risecommerce_blog_tag_store', 'tag_id', 'risecommerce_blog_tag', 'tag_id'),
                'tag_id',
                $setup->getTable('risecommerce_blog_tag'),
                'tag_id',
                \Magento\Framework\DB\Ddl\Table::ACTION_CASCADE
            )->addForeignKey(
                $setup->getFkName('risecommerce_blog_tag_store', 'store_id', 'store', 'store_id'),
                'store_id',
                $setup->getTable('store'),
                'store_id',
                \Magento\Framework\DB\Ddl\Table::ACTION_CASCADE
            )->setComment(
                'Risecommerce Blog Tag To Store Linkage Table'
            );
            $connection->createTable($table);
        }

        if (version_compare($version, '2.10.0') < 0) {
            foreach (['risecommerce_blog_tag', 'risecommerce_blog_category'] as $table) {
                $connection->addColumn(
                    $setup->getTable($table),
                    'posts_per_page',
                    [
