...
namespace Risecommerce\Blog\Model\ResourceModel\Post;
...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection
{
    ...
    /**
     * @var \Risecommerce\Blog\Api\CategoryRepositoryInterface|null
     */
     ...
     * @param \Risecommerce\Blog\Api\CategoryRepositoryInterface|null $categoryRepository
     */
    public function __construct(
        ...
        \Risecommerce\Blog\Api\CategoryRepositoryInterface $categoryRepository = null
    ) {
        ...
        $this->categoryRepository = $categoryRepository ?: $objectManager->create(
            \Risecommerce\Blog\Api\CategoryRepositoryInterface::class
        );
    }
    ...
    {
        parent::_construct();
        $this->_init(\Risecommerce\Blog\Model\Post::class, \Risecommerce\Blog\Model\ResourceModel\Post::class);
     ...
     * @param array|int|\Risecommerce\Blog\Model\Category  $category
     * @return $this
     */
    public function addCategoryFilter($category)
    {
        if (!$this->getFlag('category_filter_added')) {
            if ($category instanceof \Risecommerce\Blog\Model\Category) {
            ...
            $tableName = $this->getTable('risecommerce_blog_category');
    ...
    public function addSearchFilter($term)
    {
        ...
        $select = $connection->select()
            ->from(
                ['p' => $this->getTable('risecommerce_blog_post')],
                ['post_id']
            )->joinInner(
                ['pt' => $this->getTable('risecommerce_blog_post_tag')],
                'p.post_id = pt.post_id',
                ['']
            )->joinInner(
                ['t' => $this->getTable('risecommerce_blog_tag')],
                't.tag_id = pt.tag_id',
                ['tag_title' => 'title']
     ...
     * @param array|int|string|\Risecommerce\Blog\Model\Tag  $tag
     * @return $this
     */
    public function addTagFilter($tag)
    {
        if (!$this->getFlag('tag_filter_added')) {
            if ($tag instanceof \Risecommerce\Blog\Model\Tag) {
                $tag = [$tag->getId()];
            }
            ...
            $connection = $this->getConnection();
            $tableName = $this->getTable('risecommerce_blog_tag');
     ...
     * @param array|int|\Risecommerce\Blog\Model\Author  $author
     * @return $this
     */
    public function addAuthorFilter($author)
    {
        if (!$this->getFlag('author_filter_added')) {
            if ($author instanceof \Risecommerce\Blog\Model\Author) {
                $author = [$author->getId()];
            }
            ...
            $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
            $authorModel = $objectManager->get(\Risecommerce\Blog\Api\AuthorInterface::class);
    ...
    protected function _afterLoad()
    {
        ...
        if (count($items)) {
            $connection = $this->getConnection();
            $tableName = $this->getTable('risecommerce_blog_post_store');
            $select = $connection->select()
                ->from(['cps' => $tableName])
            ...
            foreach ($map as $key => $property) {
                $tableName = $this->getTable('risecommerce_blog_post_' . $key);
                $select = $connection->select()
                    ->from(['cps' => $tableName])
                    ->where('cps.post_id IN (?)', $items);
        ...
        foreach (['store', 'category', 'tag', 'author', 'relatedproduct'] as $key) {
            if ($this->getFilter($key)) {
                ...
                $this->_eventManager->dispatch(
                    'rcblog_post_collection_render_filter_join',
                    ['join_options' => $joinOptions]
                );
                $this->getSelect()->join(
                    [$key . '_table' => $this->getTable('risecommerce_blog_post_' . $key)],
