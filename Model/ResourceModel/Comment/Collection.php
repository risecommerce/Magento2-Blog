...
namespace Risecommerce\Blog\Model\ResourceModel\Comment;
...
class Collection extends AbstractCollection
{
    ...
    {
        $this->_init(\Risecommerce\Blog\Model\Comment::class, \Risecommerce\Blog\Model\ResourceModel\Comment::class);
    ...
    {
        return $this
            ->addFieldToFilter('status', \Risecommerce\Blog\Model\Config\Source\CommentStatus::APPROVED);
    }
     ...
     * @param array|int|string|\Risecommerce\Blog\Model\Post  $post
     * @return $this
     */
    public function addPostFilter($post)
    {
        if (!$this->getFlag('post_filter_added')) {
            if ($post instanceof \Risecommerce\Blog\Model\Post) {
                $post = [$post->getId()];
            }
            ...
            $tableName = $this->getTable('risecommerce_blog_post');
    ...
    protected function _renderFiltersBefore()
    {
        if ($this->getFilter('store') && !$this->getFlag('store_filtered')) {
            $this->getSelect()->join(
                ['store_table' => $this->getTable('risecommerce_blog_post_store')],
