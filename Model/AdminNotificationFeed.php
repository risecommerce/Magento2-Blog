...
class AdminNotificationFeed extends \Magento\AdminNotification\Model\Feed
{
    ...
    public function getFeedUrl()
    {
        ...
        $modulesParams = [];
        foreach ($this->getRisecommerceModules() as $key => $module) {
            $key = str_replace('Risecommerce_', '', $key);
    ...
    /**
     * Get Risecommerce Modules Info
     *
     * @return $this
     */
    protected function getRisecommerceModules()
    {
        $modules = [];
        foreach ($this->_moduleList->getAll() as $moduleName => $module) {
            if (strpos($moduleName, 'Risecommerce_') !== false && $this->_moduleManager->isEnabled($moduleName)) {
    ...
    public function checkUpdate()
    {
        ...
        if (($frequency + $session->getRcNoticeLastUpdate() > $time)
            || ($frequency + $this->getLastUpdate() > $time)
        ) {
            return $this;
        }

        $session->setRcNoticeLastUpdate($time);
    ...
    {
        return $this->_cacheManager->load('risecommerce_admin_notifications_lastcheck');
    }
    ...
    {
        $this->_cacheManager->save(time(), 'risecommerce_admin_notifications_lastcheck');
        return $this;
    }
}
